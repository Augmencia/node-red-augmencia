...
module.exports = function(RED) {
  function ArAssetsEventsListenerNode(config) {
    ...
    if (augmenciaServices)
    {
      ...
      node.on('input', async function(msg) {
        ...
        (async function subscribeToArAssetsEvents() {
          stopStream(stream);
          await augmenciaServices.limiter.schedule(() => new Promise((resolve, reject) => {
            streams
            node.status({fill:'green',shape:'dot',text:'node-red:common.status.connected'})
            stream.stream = augmenciaServices.projectsService.RegisterToARAssetsModifications(payload, metadata)
              ...
              .on('end', function() {
                if (streams.indexOf(stream) >= 0) {
                  stream.timeout = setTimeout(subscribeToArAssetsEvents, 5000);
                }
                if (streams.length <= 0) {
                  node.status({fill:'red',shape:'ring',text:'node-red:common.status.disconnected'});
                }
  ...
  RED.nodes.registerType("augmencia-ar-assets-events-listener", ArAssetsEventsListenerNode);
}