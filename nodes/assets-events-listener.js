...
module.exports = function(RED) {
  function AssetsEventsListenerNode(config) {
    ...
    if (augmenciaServices)
    {
      ...
      node.on('input', async function(msg) {
        ...
        (async function subscribeToAssetsEvents() {
          stopStream(stream);
          await augmenciaServices.limiter.schedule(() => new Promise((resolve, reject) => {
            streams
            node.status({fill:'green',shape:'dot',text:'node-red:common.status.connected'})
            stream.stream = augmenciaServices.projectsService.RegisterToAssetsModificationsFromProject({value: projectId}, metadata)
              ...
              .on('end', function() {
                if (streams.indexOf(stream) >= 0) {
                  stream.timeout = setTimeout(subscribeToAssetsEvents, 5000);
                }
                if (streams.length <= 0) {
                  node.status({fill:'red',shape:'ring',text:'node-red:common.status.disconnected'});
                }
  ...
  RED.nodes.registerType("augmencia-assets-events-listener", AssetsEventsListenerNode);
}