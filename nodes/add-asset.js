...
module.exports = function(RED) {
        ...
        node.on('input', async function(msg) {
          try
          {
            ...
            msg.payload = await augmenciaServices.limiter.schedule(() => new Promise(function(resolve, reject) {
              try {
                augmenciaServices.projectsService.AddAsset(msg.payload, metadata, function(err, asset) {
                  if (err) {
                    reject(err);
                  } else {
                    resolve(asset);
  ...
  RED.nodes.registerType("augmencia-add-asset",AddAssetNode);
}