...
module.exports = function(RED) {
  function DeleteProjectNode(config) {
        ...
        node.on('input', async function(msg) {
          try
          {
            ...
            await augmenciaServices.limiter.schedule(() => new Promise(function(resolve, reject) {
              augmenciaServices.projectsService.DeleteProject({value: msg.payload}, metadata, function(err, _) {
                if (err) {
                  reject(err);
                } else {
                  resolve();
  ...
  RED.nodes.registerType("augmencia-delete-project",DeleteProjectNode);
}